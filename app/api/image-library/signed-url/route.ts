...
export async function POST(request: NextRequest) {
  try {
    const { imagePath, thumb } = await request.json();
    ...
    // Generate signed URL (optionally transformed for thumbnail)
    const transformOptions = thumb
      ? {
          transform: {
            width: 512,
            height: 512,
    ...
    const { data, error } = await supabase.storage
      ...
      .createSignedUrl(imagePath, 60 * 60, transformOptions);
        ...
        if (organizationId) {
          ...
          if (filename) {
            ...
            if (
              ...
              console.log(
                `Path appears to be user-based (${firstPart}), but we need org-based access`
              );
            ...
            const { data: orgData, error: orgError } = await supabase.storage
              .from("datasheet-assets")
              .createSignedUrl(orgPath, 60 * 60, transformOptions);
