...
export default function ImageLibraryClient({ initialData }: ImageLibraryClientProps) {
  const [images, setImages] = useState<ImageItem[]>(initialData.images);
  const [filteredImages, setFilteredImages] = useState<ImageItem[]>(initialData.images);
  ...
  const applyFilters = useCallback((images: ImageItem[], filters: ImageFilters) => {
    let filtered = [...images];
  ...
  if (initialData.error) {
    return (
      <div className="text-center py-8">
        <p className="text-destructive">Error loading images: {initialData.error}</p>
      </div>
    );
  }
